...
export default {
    data: new SlashCommandBuilder()
        .setName('demote')
        .setDescription("Demotes a member from one role to another and posts an announcement.")
        ...
        .addRoleOption((option) =>
            option
                .setName('to')
                .setDescription('The new (lower) role to give them.')
                .setRequired(true),
        )
        .addRoleOption((option) =>
            option
        ...
        .addStringOption((option) =>
            option
                .setName('reason')
                .setDescription('Optional reason for the demotion.')
                .setRequired(false),
        )
    ...
    async execute(interaction) {
        ...
        if (!interaction.member.permissions.has(PermissionFlagsBits.ManageRoles)) {
            throw new TitanBotError(
                ...
                "You need the 'Manage Roles' permission to demote members.",
        ...
        try {
            ...
            logger.error(`Failed to update roles for demotion in guild ${interaction.guildId}:`, error);
            throw new TitanBotError(
        ...
        const announceEmbed = createEmbed({
            ...
            description:
                `${member} Has Been **DEMOTED** to ${toRole}` +
        ...
        await destinationChannel.send({ embeds: [announceEmbed] }).catch((sendError) => {
            logger.warn(`Could not send demotion announcement in guild ${interaction.guildId}: ${sendError.message}`);
        });

        logger.info(
            `${interaction.user.tag} demoted ${targetUser.tag} to ${toRole.name}${fromRole ? ` from ${fromRole.name}` : ''} in guild ${interaction.guildId}`,
        );
