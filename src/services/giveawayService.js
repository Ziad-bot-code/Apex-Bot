...
export async function openWinnerTickets(client, guild, giveaway, winners) {
    ...
    try {
        ...
        if (config?.winnerTicketEnabled === false) {
            logger.debug(`Winner ticket auto-open is disabled for guild ${guild.id}, skipping`);
            return [];
        }
    } catch (error) {
        logger.warn(`Could not check winnerTicketEnabled for guild ${guild.id}, defaulting to enabled:`, error.message);
    }
